 ...
 * @return mix 
 */
if (typeof window.langSlider === 'undefined') {
    window.langSlider = class {
    a = '安全验证';
    b = {};
    c = false;
    d = 3;
    x = 27;
    f = '拖动滑块完成拼图，如有问题请联系管理员';
    g = '验证成功，登录中';
    h = '是不是太难了，咱换一个';
    i = '拖动滑块完成拼图';
    j = false;
    ...
    init() {
        ...
        var sliderClose = document.querySelector('#sliderClose');
        sliderClose.onclick = (e)=>{
            that.c = false;
            that.j = false;
            $('.lang-dialog__wrapper').fadeOut(200,function (){
                $(this).remove();
            });
        }
        ;
    ...
    var slider = new window.langSlider({
        ...
        start: function(that) {
            $.ajax({
                ...
                dataType: 'json',
                url: Theme.ajaxurl,
                data: { action: 'get_ppoc_data' },
                success: function(data) {
                    if (data.img && data.key) {
                        that.si = data.img;
                        that.sk = data.key;
                        that.y = data.y;
                        that.sliderReset({
